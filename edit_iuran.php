...
if (isset($_GET['Id_bayar'])) {
    ...
    $stmt = $link->prepare("SELECT * FROM iuran WHERE Id_bayar = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Update data 
    $stmt = $link->prepare("UPDATE iuran SET Nama = ?, No_Kamar = ?, Tunggakan = ?, Total_bayar = ?  WHERE Id_bayar = ?");
    $stmt->bind_param("sssss", $Nama, $No_Kamar, $Tunggakan, $Total_bayar, $Id_bayar);

    if ($stmt->execute()) {
        echo "<script>alert('Data berhasil diperbarui.'); window.location.href = 'iuran.php';</script>";
    } else {
        echo "<script>alert('Terjadi kesalahan saat memperbarui data .'); window.location.href = 'iuran.php';</script>";
    }
}
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form action="" method="post">
                        ...
                        <a href="iuran.php" class="btn btn-warning">Kembali</a>
                    </form>
                </div>
            </div>
        </div>
